<template>
  <div class="container-filter">
    ...
    <div id="filter-price">
      <select id="select-price" v-model="selected">
        ...
        <option v-for="filter in filtersPrice" :value="filter" :key="filter">
          {{ filter }}
        </option>
      </select>
    </div>
  ...
  <section class="cards">
    <article
      class="card"
      v-for="(imovel, index) in imoveisFiltered"
      :key="index"
    >
      ...
      <div class="container-card">
        ...
        <p>
          {{ imovel.bedrooms }}QT {{ imovel.suites }}ST
          {{ imovel.parking_spots }}VG
        </p>
      </div>
    </article>
  </section>
  ...
  methods: {
      ...
      res = res.sort(function (a, b) {
        ...
        if (order)
          return parseInt(previous) < parseInt(next)
            ? -1
            : parseInt(previous) > parseInt(next)
            ? 1
            : 0;
        else
          return parseInt(previous) > parseInt(next)
    ...
    formatPrice(data) {
      Object.keys(data).forEach((i) => {
        ...
        if (price.length === 6) {
          ...
          price =
            priceSplit[0].substr(0, 1) +
            "." +
            priceSplit[0].substr(1, 3) +
            "." +
            priceSplit[1];
        }
    ...
    mapPicture(dataImoveis) {
      Object.keys(dataImoveis).forEach((i) => {
        if (
          dataImoveis[i].property_type === "Apartamento" ||
          dataImoveis[i].property_type === "Cobertura"
        ) {
          dataImoveis[i].picture =
            "https://uploaddeimagens.com.br/images/003/701/304/full/edificio.png?1643739307";
        } else if (dataImoveis[i].property_type === "Casa de Condomínio") {
          dataImoveis[i].picture =
            "https://uploaddeimagens.com.br/images/003/701/336/full/imovel.png?1643740111";
        } else if (dataImoveis[i].property_type === "Casa") {
          dataImoveis[i].picture =
            "https://uploaddeimagens.com.br/images/003/701/327/full/casa.png?1643739984";
        }
      });
    },
  },
